...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['withdrawal_id'])) {
    ...
    $stmt->bindValue(':withdrawal_id',  $withdrawal_id, PDO::PARAM_INT);
    $stmt->execute();
...
// Retrieve all withdrawal records along with user names
$sql = "
    SELECT
        w.withdrawal_id,
        w.user_id,
...
<head>
  ...
  <title>Admin Withdrawal - Benefit Market Trade</title>
    ...
    <div class="flex-1 flex flex-col overflow-auto">
      ...
      <main class="p-6">
        <div class="container mx-auto">
          <h1 class="text-2xl font-bold mb-4">Admin Withdrawals Management</h1>
          ...
          <!-- Wrap the table to prevent overflow -->
          <div class="overflow-x-auto">
            <table class="min-w-full bg-white">
              <thead class="bg-gray-800 text-white">
                <tr>
                  <th class="py-3 px-4">Withdrawal ID</th>
              ...
              <tbody class="text-gray-700">
                <?php foreach($withdrawals as $withdrawal): ?>
                  <tr class="border-b">
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['withdrawal_id']); ?></td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['user_id']); ?></td>
                    <td class="py-3 px-4">
                      <?php echo htmlspecialchars($withdrawal['fname'] . ' ' . $withdrawal['lname']); ?>
                    </td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['crypto_type']); ?></td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['amount']); ?></td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['wallet_address']); ?></td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['created_at']); ?></td>
                    <td class="py-3 px-4"><?php echo htmlspecialchars($withdrawal['status']); ?></td>
                    <td class="py-3 px-4">
              ...
              data-withdrawal='<?php 
                echo htmlspecialchars(
                  ...
                  ENT_QUOTES,  // escape both single & double quotes
                  'UTF-8'
                ); 
              ?>'
            >Update</button>
          ...
          <div id="updateModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
            <div class="bg-white rounded p-6 w-1/3">
              <h2 class="text-xl font-bold mb-4">Update Withdrawal</h2>
              <form method="POST">
                <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                <input type="hidden" name="withdrawal_id" id="modal_withdrawal_id">
                <div class="mb-4">
                  <label for="status" class="block text-gray-700">Status</label>
                  <textarea name="status" id="modal_status" rows="3" class="mt-1 block w-full border-gray-300 rounded"></textarea>
                </div>
        ...
        <script>
          ...
          document.querySelectorAll('.update-btn').forEach(button => {
            button.addEventListener('click', function() {
              ...
              document.getElementById('modal_withdrawal_id').value = withdrawal.withdrawal_id;
              document.getElementById('modal_status').value = withdrawal.status;
