...
try {
    // 1) Get total notifications count for pagination
    $count_sql = "
      SELECT COUNT(*) AS total FROM (
        SELECT trade_date       AS activity_date FROM trades       WHERE user_id = :uid
        UNION ALL
        SELECT activity_date    AS activity_date FROM activity_log  WHERE user_id = :uid
        UNION ALL
        SELECT created_at       AS activity_date FROM deposits     WHERE user_id = :uid
      ) AS combined
    ";
    ...
    // 2) Fetch the paginated activity feed
    $main_sql = "
    ...
    // Bind parameters, ensuring ints for limit/offset
...
// Now $activities contains each ['activity_date','activity']
// and you have $totalPages to render your pagination links.
?>
...
<div class="flex h-screen">
  ...
  <div class="flex-1 flex flex-col overflow-auto">
        ...
        <div class="relative">
          ...
          <div id="notificationDropdown" class="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded shadow-lg hidden z-50">
            <div class="p-4">
              ...
              <ul class="space-y-4 text-gray-900">
                <?php
                // Desktop notifications from main query result
    ...
    <main class="p-6 space-y-8 bg-gray-50 min-h-screen">
      <!-- Recent Notifications Section (Main Content) -->
      <section class="mt-8 max-w-4xl mx-auto bg-gray-800 shadow-xl rounded-xl p-6">
        <h4 class="text-2xl font-bold text-gray-100 mb-4">Recent Notifications</h4>
        <ul class="space-y-4 text-gray-300">
          <?php
          // Re-run the main query to output the notifications in the main content
          $stmt_main->execute();
          $result_activity = $stmt_main->get_result();
          if ($result_activity->num_rows > 0) {
              while ($activity = $result_activity->fetch_assoc()) {
                  echo '<li class="border-b border-gray-700 pb-2">';
                  echo '<p class="text-sm">' . htmlspecialchars($activity["activity"]) . '</p>';
      ...
      <div class="max-w-4xl mx-auto mt-4 flex justify-center">
        <nav class="inline-flex -space-x-px">
          <?php if ($page > 1): ?>
            <a href="?page=<?php echo $page - 1; ?>" class="px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700">Previous</a>
          <?php endif; ?>
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="?page=<?php echo $i; ?>" class="px-3 py-2 leading-tight <?php echo ($i == $page) ? 'text-blue-600 border border-blue-300 bg-blue-50' : 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700'; ?>">
              <?php echo $i; ?>
            </a>
          <?php endfor; ?>
          <?php if ($page < $totalPages): ?>
            <a href="?page=<?php echo $page + 1; ?>" class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700">Next</a>
          <?php endif; ?>
        </nav>
      </div>
    </main>
